...
sequencing_coverage_plot <- function(read_count_dt,
                                     columns,
                                     info_file) {

  x <- list(data = data.table::data.table(screen = columns),
            n_constructs = nrow(read_count_dt))

  if (!missing(info_file)) {
    x$data <- merge(x$data, info_file, by = "screen", all.x = T)
  }

  x$data$abs_read_count <- sapply(columns, \(.s) {read_count_dt[, sum(get(.s))]})
